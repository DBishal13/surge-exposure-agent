# Day 2 - Context Engineering and Vector Search

## Setup

1. Upload `knowledge_docs/` to a Unity Catalog volume, e.g.
   `/Volumes/main/surge_exposure/knowledge_docs` (Catalog Explorer > your
   catalog > Create volume, then upload the `.md` files, or
   `databricks fs cp -r knowledge_docs dbfs:/Volumes/main/surge_exposure/knowledge_docs`).
...
## What this covers (Day 2 practical outcome)

- `knowledge_docs/` — five docs pulled from the real
...
initial analysis bug, the inert flood term). That's the point: the Day 3
agent should be able to answer "how good is this score, really?" honestly.

The resulting index (`<catalog>.<schema>.knowledge_chunks_index`) is what
`../day3-agent`'s `search_methodology` tool queries.
